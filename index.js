  ...
  try {
    singleUpload1(req, res, function (err) {
      if (err) {
        ...
        const imgLocation = path.join(req.file.filename);
        Department.create({
          name: req.body.name,
          img: imgLocation,
          slug: req.body.slug,
...
app.post("/addNewEmployee", (req, res) => {
  try {
    singleUpload1(req, res, async function (err) {
      if (err) {
        return res.status(500).json({ message: err.message });
      }

      if (req.body.ref_id != null) {
        const checkEmp = await EmployeeSchema.find({ ref_id: req.body.ref_id });
        const imgLocation = path.join(req.file.filename);
...
app.post("/addNewEmployee1", async (req, res) => {
  try {
    if (req.body.ref_id != null) {
      const checkEmp = await EmployeeSchema.find({ ref_id: req.body.ref_id });
...
const multiupload = multer({ storage: storage }).array("images", 2);
app.post("/addPostionsCandidate", (req, res) => {
  try {
    multiupload(req, res, function (err) {
      if (err) {
        res.status(500).json({ message: err.message });
      } else {
        const imgLocationCV = path.join(req.files[0].filename);
        const imgLocationImage1 = path.join(req.files[1].filename);
...
app.post("/create_expense", async (req, res) => {
  try {
    ...
    const result = await ExpenseRecord.create({
      ...
      employee_office: expense.employee_office,
    });
...
app.post("/issued_enventory", async (req, res) => {
  try {
    ...
    const updatedData = await Inventoryitem.findOneAndUpdate(
      { _id: req.body.itemId },
      { $inc: { availableItem: -req.body.quantity } },
      { new: true }
    );
...
app.get("/issued", async (req, res) => {
  try {
    ...
    const branchPromises = enventory.map(async (item) => {
      const branch = await Branch.findById(item.branch_name);
      return branch
        ? { ...item.toObject(), branch_name: branch.branch_name }
        : item.toObject();
    });

    const enventoryWithBranch = await Promise.all(branchPromises);

    res.status(200).json(enventoryWithBranch);
  } catch (error) {
    console.error("Error fetching Enventory data:", error);
    res.status(500).json({ message: error.message });
  }
});
...
app.post("/add-item", async (req, res) => {
  try {
    ...
    const item_name = capitalizeFirstLetter(req.body.item_name);

    if (!branch_name) {
      return res.status(400).json({ message: "Branch name is required." });
    }

    // Check if a record with the same branch_name and item_name exists
    ...
    if (!existingRecord) {
      ...
      // If a record with the same branch_name and item_name exists, update it
      await Showrecord.findOneAndUpdate(
        { item_id: existingRecord._id, branch_name: branch_name },
        { $inc: { quantity: req.body.quantity } }
      );

      const updatedData = await Inventoryitem.findOneAndUpdate(
        { _id: existingRecord._id, branch_name: branch_name },
        {
          $inc: {
            availableItem: req.body.quantity,
            quantity: req.body.quantity,
            lossDamageItem: req.body.lossDamageItem,
          },
          $set: {
      ...
      if (!updatedData) {
        return res.status(404).json({
          message: `Cannot find any product with Item Name ${item_name} and Branch Name ${branch_name}`,
        });
      }
      res.status(200).json(true);
    }
...
//       const { item_name, quantity, lossDamageItem, category_id } = req.body;
...
//         });
//       } else {
//         // If record with the same setSuggestedId and branch_name exists, update it
//         await Showrecord.findOneAndUpdate(
//           { item_id: setSuggestedId, branch_name: branch_name },
//           { $inc: { quantity: quantity } }
//         );

//         const updatedData = await Inventoryitem.findOneAndUpdate(
//           { _id: setSuggestedId, branch_name: branch_name },
//           {
//             $inc: {
...
//         if (!updatedData) {
//           return res.status(404).json({
//             message: `Cannot find any product with ID ${setSuggestedId} and branch ${branch_name}`,
//           });
//         }
//         res.status(200).json(true);
//       }
...
app.get("/getItem", async (req, res) => {
  try {
    ...
    const branchIds = inventoryItems.map((item) => item.branch_name);
    ...
    const responseData = await Promise.all(
      inventoryItems.map(async (item) => {
        ...
        const branch = BranchData.find((branch) =>
          branch._id.equals(item.branch_name)
        );
        ...
        return {
          ...
          branch_name: branch ? branch.branch_name : "Branch Deleted",
...
app.get("/getItemrecord", async (req, res) => {
  try {
    ...
    // Assuming Showrecord has a property named 'branch_id'
    const branchIds = inventoryitem.map((item) => item.branch_name);

    // Fetch branch information for all branch_ids
    const branches = await Branch.find({ _id: { $in: branchIds } }).sort({
      createdAt: -1,
    });

    // Merge branch information into inventoryitem
    const mergedItems = inventoryitem.map((item) => {
      const branchInfo = branches.find((branch) =>
        branch._id.equals(item.branch_name)
      );
      return {
        ...item.toObject(),
        branch_name: branchInfo ? branchInfo.branch_name : null,
      };
    });
...
app.post("/addTorepair", async (req, res) => {
  try {
    ...
    if (repair_status !== "completely_demage") {
      // Update or create the EnventoryRepair record
      ...
      const updatedAccount = await LossDamageItem.findOneAndUpdate(
        { serial_number: serial_number },
        { $set: { repair_status: repair_status, comment: comment } },
...
app.get("/items/getdata", async (req, res) => {
  try {
    const query = req.query.query;
...
const singleUpload1 = multer({ storage: storage }).single("image");
...
app.post("/update_clientAccount", upload1.single("image"), async (req, res) => {
  try {
    const {
      id,
      client_name,
    ...
    if (req.file) {
      const imgLocation = path.join(req.file.filename);
      // Use the local path instead of S3 location
...
app.delete("/delete_employeeexit/:id", async (req, res) => {
  try {
    ...
    const docsToDelete = await EmployeeExitDocs.find({ ref_id: id });
    // Delete documents from the EmployeeExitDocs collection
    ...
    if (result2.deletedCount >= 1) {
      ...
      docsToDelete.forEach((doc) => {
        const filePath = path.join(__dirname, "images", doc.url);
        fs.unlink(filePath);
      });
      const result1 = await EmployeeExit.deleteOne({ _id: objectId });
      res.status(200).json({ message: "Deletion Successful" });
      // Delete the record from the EmployeeExit collection
    } else {
      const result1 = await EmployeeExit.deleteOne({ _id: objectId });
...
app.delete("/delete_branch/:id", async (req, res) => {
  try {
    const { id } = req.params;
